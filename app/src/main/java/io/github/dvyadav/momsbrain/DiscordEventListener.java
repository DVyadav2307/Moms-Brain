...
public class DiscordEventListener extends ListenerAdapter {
  ...
  public void onReady(ReadyEvent event) {
    ...
    event
        .getJDA()
        .getGuildsByName("Mom's Basement", false)
        .getFirst()
        .updateCommands()
        .addCommands(MomBasementSlashCommandManager.getCommandsAsList())
        .queue();
  ...
  public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
    ...
    if (event.getName().equals("push_notes")) {
      ...
      try {
        ...
        // uploading the file
        DriveResourceManager.uploadFile(
            file.getUrl(),
            file.getFileName(),
            file.getContentType(),
            topics,
        ...
        hook.sendMessage("Uploaded Successfully!. \nThanks for sharing the resource with others.")
            .queue();
        event
            .getChannel()
            .sendMessage(
                "@everyone New material added in "
                    + subject
                    + " by "
    ...
    if (event.getName().equals("update_notes")) {
      ...
      try {
        ...
        subjFolders =
            DriveResourceManager
                .getMostRecentFolderList(); /* TODO:monitor behaviour and use getLatestFoldersList() if required */
        // prepare the names in one string
        StringBuilder subjectNames = new StringBuilder();
        for (File file : subjFolders) subjectNames.append("\n * ").append(file.getName());
        // send list names
        hook.sendMessage(
                "Updated Successfully!! \nFollowing subject notes are available:-" + subjectNames)
            .queue();
      } catch (IOException e) {
        e.printStackTrace();
        hook.sendMessage("Some error occurred! Inform the admin.").setEphemeral(true).queue();
      }
      return;
    }
    // Interaction code
    if (event.getName().equals("show_time")) {
      ...
      if (Objects.requireNonNull(event.getOption("really")).getAsBoolean()) {
        ...
        DateTimeFormatter timeFormatter =
            DateTimeFormatter.ofPattern("HH:mm:ss"); // Hours, Minutes, Seconds
        DateTimeFormatter dateFormatter =
            DateTimeFormatter.ofPattern("dd-MM-yyyy"); // Day, Month, Year
  ...
  public void onCommandAutoCompleteInteraction(CommandAutoCompleteInteractionEvent event) {
    ...
    // pull and display the names of the subjects from drive when a user uses "pull_notes" or
    // "push_notes" command
    if ((event.getName().equals("pull_notes") || event.getName().equals("push_notes"))
      ...
      try {
        ...
        // filter folders matching with a search filed
        // map to a new Command.Choice object to create a new folder list
        List<Command.Choice> optionChoice =
            subjFolders.stream()
                .filter(
                    folder ->
                        folder
