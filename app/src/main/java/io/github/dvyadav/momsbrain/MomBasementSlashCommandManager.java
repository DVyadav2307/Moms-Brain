...
public class MomBasementSlashCommandManager {
  ...
  // TODO: Urgent need to create a delete notes command (available only to admin role)
  ...
  private static void prepareCommands() {
    ...
    cmdList.add(Commands.slash("update_notes", "Updates the subjects and notes from bot's repo"));
    cmdList.add(
        Commands.slash("pull_notes", "Get download links of subject notes.")
            .addOption(OptionType.STRING, "subject", "select/type subject", true, true));
    cmdList.add(
        Commands.slash("push_notes", "Upload notes and be a good boy/girl")
            .addOption(
                OptionType.STRING,
                "subject",
                    "Select the subject of notes. If not in list then select 'Others' and inform the admin",
                true,
                true)
            .addOption(OptionType.ATTACHMENT, "attachment", "Attach the notes", true)
            .addOption(
                OptionType.STRING, "topics", "Enter the topic names separated by commas.", true));
