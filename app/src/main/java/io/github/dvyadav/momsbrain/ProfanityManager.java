...
public class ProfanityManager {
    ...
    if (isChatProfane(chatMsg.getContentStripped())) {
      ...
      String kickedUserDMReply =
          "Hey "
              ...
              + serverOwner.getAsMention()
              + " for help."
              + "\nThanks for understanding!:pray:";
      String kickedUserReportToOwner =
          ":no_entry:"
              + user.getName()
              + " is Kicked!!"
              + "\nAt Time: "
      ...
      String[] warnMsg = {
        ":warning: Watch Your Words " + user.getEffectiveName() + "! I am counting.",
        ":warning: Mind your language! You wont get many chances.",
        ":warning:What did you just said?"
      };
      String warnReportToOwner =
          ":warning:"
              + user.getName()
              + " abused: \""
              + chatMsg.getContentDisplay()
              + "\""
              + "\nOn Channel: "
              + chatMsg.getChannel().getName()
              + "\nAt Time: "
              + msgTimeDate
              + "\n No. of times: "
              + (getUserAbuseCount(user.getIdLong()) + 1);
      ...
      chatMsg
          .reply(warnMsg[(int) (Math.random() * 3)])
          .queue(
              e -> {
                e.addReaction(
                        Emoji.fromFormatted(
                            "U+003" + getUserAbuseCount(user.getIdLong()) + " U+FE0F U+20E3"))
                    .queue();
                serverOwner
                    .getUser()
                    .openPrivateChannel()
  ...
  private boolean isChatProfane(String chatMsgString) {
    ...
    String[] words =
        chatMsgString /* preloaded with removed text formatting (see method call)*/
  ...
  private void increaseUserAbuseCount(Long userId) {
    ...
    else abuseRecords.put(userId, 1);
  }
